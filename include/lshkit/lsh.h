...
class StableDistLsh
{
    std::vector<float> a_;	// Store random values from Gaussian distribution
    float b_;	// Random in range (0, W_)
    float W_;	// Window size =?
    unsigned dim_;	// n-dimensions
public:
    /**
    ...
    typedef const float *Domain;	// n-dimensions feature vector
...
//            }
        ...
        return unsigned(int(std::floor(ret / W_)));	// Transform a feature vector to a integer
    }
