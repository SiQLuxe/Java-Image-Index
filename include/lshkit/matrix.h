...
public:
    ...
    void reset (int _dim, int _N)
    {
        ...
        dims = new T[dim * N];	// N*dim float array
    }
    ...
    public:
        ...
        Accessor(const Matrix &matrix)
            : matrix_(matrix), flags_(matrix.getSize()) {}	// N bits in flags_ filling with 0
        // FloatMatrix: Matrix<float>.getSize() == N(Number of feature vectors)

        void reset () {
            flags_.reset();	// fill bit with 0 in flags_
        }

        bool mark (unsigned key) {
            if (flags_[key]) return false;
            flags_.set(key);	// set the keyth position with 1
            return true;
        }

        const float *operator () (unsigned key) {
            return matrix_[key];	// return keyth feature vector
        }
    };
