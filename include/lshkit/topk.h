...
/**
  ...
  */
template <class KEY> // KEY: unsigned
...
public:
    /// Key type.
    typedef typename ACCESSOR::Key Key;	// Key type: unsigned
    /// Value type.
    typedef typename ACCESSOR::Value Value;	// Value type: const float *
    ...
    /**
      ...
      */
    void reset (Value query) {	// query value(Domain): const float *
        query_ = query;	// Domain: const float *
        accessor_.reset();
        topk_.reset(K_, R_);
        cnt_ = 0;
    }
    ...
    void operator () (Key key) {
        if (accessor_.mark(key)) {
            ++cnt_;
            topk_ << typename Topk<Key>::Element(key, metric_(query_, accessor_(key)));	// key: ID; metric: Distance
        }
    }
private:
    ACCESSOR accessor_;
