...
/**
  ...
  * @param KEY key type.
  */
template <typename LSH, typename KEY>	// LSH: LSB<StableDistLsh<Gaussian>>, KEY: unsigned
class ForestIndex
{
    ...
    struct Tree
    {
                ...
                if (children.empty()) {
                    ...
                    // split
                        ...
                        BOOST_FOREACH(Key key, data) {
                            unsigned h = lsh(acc(key));	// matrix.h Accessor: keyth feature vector; h = 0, 1
                            if (children[h] == 0) {
                                children[h] = new Node();
                            }
        ...
        void reset (const Parameter &param, ENGINE &engine, unsigned depth)
        {
            ...
            BOOST_FOREACH(LSH &h, lsh) {
                h.reset(param, engine);	// composite.h LSB->FixedTail->StableDistLsh(lsh.h)
            }
            root = new Node();
        }
        ...
        void lookup (Domain val, std::vector<const Node *> *nodes) const {
            ...
            for (;;) {
                ...
                unsigned h = lsh[depth](val);	// h = 0,1
                cur = cur->children[h];
                if (cur == 0) break;
                ++depth;
            }
...
public:
    ...
    /**
      ...
      */
    template <typename SCANNER>	// SCANNER: TopkScanner<FloatMatrix::Accessor, metric::l2<float> >
    void query (Domain val, unsigned M, SCANNER &scanner) const
    {
        ...
        for (unsigned i = 0; i < trees.size(); ++i) {
            trees[i].lookup(val, &list[i]);	// search to bottom
        }
