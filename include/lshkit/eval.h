...
class Benchmark
{
    unsigned Q_;	// number of queries

    std::vector<unsigned> queries_;	// query IDs
    std::vector<Topk<KEY> > topks_;	// KNN points and distance corresponding to a query ID
public:
    Benchmark(): Q_(0) {}
    ...
    void resize(unsigned Q, unsigned K = 0)
    {
        ...
        if (K > 0) {
            BOOST_FOREACH(Topk<KEY> &knn, topks_) {
                ...
                knn.resize(K);	// topk.h Topk<unsigned> -> KNN tops
            }
        }
    }
    ...
    void load (std::istream &is)
    {
        ...
        for (;;) {
            ...
            is >> q;
            if (!is) break;
            queries_.push_back(q);	// feature vector's ID(q)
            is >> k;
            topks_.push_back(Topk<KEY>());
            topks_.back().resize(k);	// inherit std::vector<TopkEntry<KEY> >
    ...
    unsigned getQ () const { return Q_; }	// Number of query
