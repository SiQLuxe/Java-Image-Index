...
public:
    /// The base LSH class.
    typedef LSH Super;	// lsh.h: StableDistLsh<Gaussian>
    /**
     * Parameter to Tail.
     */
    struct Parameter: public Super::Parameter {
 ...
 */
template <typename LSH, unsigned range>	//LSH: StableDistLsh<Gaussian>
class FixedTail
{
    BOOST_CONCEPT_ASSERT((LshConcept<LSH>));
protected:
    LSH lsh_;	// LSH: StableDistLsh<Gaussian> or StableDistLsh<Cauchy>...
public:
    typedef LSH Super;
     ...
     */
    struct Parameter: public Super::Parameter {	// range...
    };

    typedef typename Super::Domain Domain;	// typename: Domain is a class or struct; typedef const float *Domain
    ...
    unsigned operator () (Domain obj) const	//typedef const float *Domain
    {
        return lsh_(obj) % range;	// 0, 1
    }
 ...
 */
template <typename LSH>	//LSH: StableDistLsh<Gaussian>
