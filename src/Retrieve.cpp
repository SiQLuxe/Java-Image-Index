...
// Index
string features_index = "features.index";
...
void extractFeature(string query_img) {
	/**
	 * Extract feature from a image
	 */
	Mat img;
	img = imread(query_img, 0);
	GetInvar gi;
	if (!gi.IsVaild(img))
		return;
	if (img.cols > PROPORT)
...
JNIEXPORT void JNICALL Java_Retrieve_retrieve
  (JNIEnv *env, jobject obj, jstring imgn){
  	const char* c_imgn = env->GetStringUTFChars(imgn,NULL);
	if(c_imgn==NULL)
		return;
	const string s_imgn(c_imgn);
	
	loadData();
	extractFeature(s_imgn);
	Index &index = constructIndex();
	search(index);
	evaluation(index);
	
	env->ReleaseStringUTFChars(imgn,c_imgn);
